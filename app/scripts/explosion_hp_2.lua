...
function tick(p_EntityRef, p_DeltaT)
  ...
  if initialized_hp_2 == 0 then
	posX = position.x
	posZ = position.z
	initialized_hp_2 = 1
  end
  
  iTime_hp_2 = iTime_hp_2 + p_DeltaT
 
  if iTime_hp_2 > 6.0 then
    iTime_hp_2 = 2.0
	position.x = posX + math.random(-500.0, 500.0)
	position.z = posZ + math.random(-500.0, 500.0)
	spawnCounter_hp_2 = spawnCounter_hp_2 + 1
	position.y = -5.0 * size.x
  end
  
  if spawnCounter_hp_2 % modulus_hp_2 == 0 then
