...
//layout(binding = 1) uniform image2D _NormalTex;
layout(binding = 1, r11f_g11f_b10f) uniform image2D _NormalTex;
//layout(binding = 1, RGf) uniform image2D _NormalTex;
//layout(binding = 1, RGBA16f) uniform image2D _NormalTex;
//layout(binding = 1, RGBA8) uniform image2D _NormalTex;
...
void main()
{
	...
	*/
	...
	for (int x = 0; x < 32; x++)
		for (int y = 0; y < 32; y++)
			{
				imageStore(_PBRTex, id.xy + 64 * ivec2(x, y), vec4(0.0, 0.0, 1.0, 1.0));
			}
	*/
}
