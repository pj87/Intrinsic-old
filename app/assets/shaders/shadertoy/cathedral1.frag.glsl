...
vec3 getrendersample ( vec3 ro, vec3 rd ) {
    ...
    if ( rl > -0.5 ) {
        ...
        vec3 c = vec3( max(dot(ld,nor),0.0)) * li;									//Lambertian diffuse + distance falloff
        c *= softshadow(xyz, ld, ldist);											//soft shadows
        c += MAT_COL*100.0/((ldist+COLUMN_DIST*2.0)*(ldist+COLUMN_DIST*2.0));		//first light bounce approximation
        c += MAT_COL*MAT_COL*100.0/((ldist+COLUMN_DIST*4.0)*(ldist+COLUMN_DIST*4.0));	//second light bounce approximation
        c *= MAT_COL;																//multiply by light colour
